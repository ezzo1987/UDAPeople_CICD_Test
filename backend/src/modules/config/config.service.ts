...
export class ConfigService {
  ...
  private validateInput(envConfig): EnvConfig {
    const envVarsSchema: Joi.ObjectSchema = Joi.object({
      ...
      TYPEORM_ENTITIES: Joi.string().required().default('./modules/domain/**/*.entity{.ts,.js}'),
      TYPEORM_USERNAME: Joi.string().required().default('postgres'),
      TYPEORM_PASSWORD: Joi.string().required().default('hihiAndhi1'),
      TYPEORM_DATABASE: Joi.string().required().default('postgres'),
      TYPEORM_HOST: Joi.string().required().default('udapeople-prod.cs9hpqwsqqjn.us-east-1.rds.amazonaws.com'),
      TYPEORM_PORT: Joi.number()
        .integer()
        .default(5432),
      TYPEORM_MIGRATIONS: Joi.string().default('./migrations/*{.ts,.js}'),
